...
l_mem luaO_applyparam(lu_byte p, l_mem x) {
  ...
  if (e >= 0) {
    ...
    else /* real overflow */ return MAX_LMEM;
  } else { /* negative exponent */
    e = -e;
    if (x < MAX_LMEM / 0x1F) /* multiplication cannot overflow? */
      return (x * m) >> e;   /* multiplying first gives more precision */
    else if ((x >> e) < MAX_LMEM / 0x1F) /* cannot overflow after shift? */ return (x >> e) * m;
    else /* real overflow */ return MAX_LMEM;
  }
}
  ...
  for (s += 2;; s++) {                                      /* skip '0x' and read numeral */
    if (*s == dot) {
      ...
      if (sigdig == 0 && *s == '0') /* non-significant digit (zero)? */
        nosigdig++;
      else if (++sigdig <= MAXSIGDIG) /* can read it without overflow? */ r = (r * l_mathop(16.0)) + luaO_hexavalue(*s);
...
int luaO_utf8esc(char *buff, unsigned long x) {
  ...
  else { /* need continuation bytes */ unsigned int mfb = 0x3f; /* maximum that fits in first byte */
...
unsigned luaO_tostringbuff(const TValue *obj, char *buff) {
  ...
  if (ttisinteger(obj)) len = lua_integer2str(buff, LUA_N2SBUFFSZ, ivalue(obj));
  else len = tostringbuffFloat(fltvalue(obj), buff);
...
static void pushbuff(lua_State *L, void *ud) {
  ...
  case 2:                                                                         /* length overflow: Add "..." at the end of result */
    if (buff->buffsize - buff->blen < 3) strcpy(buff->b + buff->blen - 3, "..."); /* 'blen' must be > 3 */
    else { /* there is enough space left for the "..." */ strcpy(buff->b + buff->blen, "...");
      buff->blen += 3;
    }
    /* FALLTHROUGH */
...
static const char *clearbuff(BuffFS *buff) {
  ...
  else res = getstr(tsvalue(s2v(L->top.p - 1)));
...
void luaO_chunkid(char *out, const char *source, size_t srclen) {
  ...
  if (*source == '=') {        /* 'literal' source */
    ...
    else { /* truncate it */ addstr(out, source + 1, bufflen - 1);
      *out = '\0';
    }
  } else if (*source == '@') { /* file name */
    if (srclen <= bufflen)     /* small enough? */
      memcpy(out, source + 1, srclen * sizeof(char));
    else { /* add '...' before rest of name */ addstr(out, RETS, LL(RETS));
      bufflen -= LL(RETS);
