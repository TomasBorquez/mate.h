...
static int math_abs(lua_State *L) {
  if (lua_isinteger(L, 1)) {
    ...
    lua_pushinteger(L, n);
  } else lua_pushnumber(L, l_mathop(fabs)(luaL_checknumber(L, 1)));
  return 1;
}
...
static int math_toint(lua_State *L) {
  ...
  if (l_likely(valid)) lua_pushinteger(L, n);
  else {
    luaL_checkany(L, 1);
...
static int math_floor(lua_State *L) {
  if (lua_isinteger(L, 1)) lua_settop(L, 1); /* integer is its own floor */
  else {
...
static int math_ceil(lua_State *L) {
  if (lua_isinteger(L, 1)) lua_settop(L, 1); /* integer is its own ceiling */
  else {
...
static int math_fmod(lua_State *L) {
  ...
  } else lua_pushnumber(L, l_mathop(fmod)(luaL_checknumber(L, 1), luaL_checknumber(L, 2)));
  return 1;
}
...
static int math_log(lua_State *L) {
  ...
  lua_Number res;
  if (lua_isnoneornil(L, 2)) res = l_mathop(log)(x);
  else {
    lua_Number base = luaL_checknumber(L, 2);
#if !defined(LUA_USE_C89)
    if (base == l_mathop(2.0)) res = l_mathop(log2)(x);
    else
#endif
...
static int math_type(lua_State *L) {
  if (lua_type(L, 1) == LUA_TNUMBER) lua_pushstring(L, (lua_isinteger(L, 1)) ? "integer" : "float");
  else {
    luaL_checkany(L, 1);
    luaL_pushfail(L);
  }
...
static lua_Number I2d(Rand64 x) {
  ...
  if (sx < 0) res += l_mathop(1.0); /* correct the two's complement if negative */
