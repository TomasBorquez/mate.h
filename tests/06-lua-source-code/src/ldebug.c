...
LUA_API const char *lua_getlocal(lua_State *L, const lua_Debug *ar, int n) {
  ...
  if (ar == NULL) {                      /* information about non-active function? */
    if (!isLfunction(s2v(L->top.p - 1))) /* not a Lua function? */
      name = NULL;
    else /* consider live variables at function start (parameters) */ name = luaF_getlocalname(clLvalue(s2v(L->top.p - 1))->p, n, 0);
...
static int nextline(const Proto *p, int currentline, int pc) {
  if (p->lineinfo[pc] != ABSLINEINFO) return currentline + p->lineinfo[pc];
...
static void collectvalidlines(lua_State *L, Closure *f) {
  if (!LuaClosure(f)) {
    ...
    if (p->lineinfo != NULL) { /* proto with debug information? */
      ...
      else { /* vararg function */ lua_assert(GET_OPCODE(p->code[0]) == OP_VARARGPREP);
...
static const char *getfuncname(lua_State *L, CallInfo *ci, const char **name) {
  ...
  if (ci != NULL && !(ci->callstatus & CIST_TAIL)) return funcnamefromcall(L, ci->previous, name);
...
static int auxgetinfo(lua_State *L, const char *what, lua_Debug *ar, Closure *f, CallInfo *ci) {
  ...
  for (; *what; what++) {
    ...
    case 'r': {
      if (ci == NULL || !(ci->callstatus & CIST_HOOKED)) ar->ftransfer = ar->ntransfer = 0;
      else {
  ...
  *ppc = pc = findsetreg(p, pc, reg);
  if (pc != -1) { /* could find instruction? */
    ...
    case OP_MOVE: {
      ...
      if (b < GETARG_A(i)) return basicgetobjname(p, ppc, b, name); /* get name for 'b' */
      break;
    }
    case OP_GETUPVAL: {
...
static const char *isEnv(const Proto *p, int pc, Instruction i, int isup) {
  ...
  if (isup)            /* is 't' an upvalue? */
    name = upvalname(p, t);
  else { /* 't' is a register */ const char *what = basicgetobjname(p, &pc, t, &name);
    /* 'name' must be the name of a local variable (at the current
       level or an upvalue) */
    if (what != strlocal && what != strupval) name = NULL; /* cannot be the variable _ENV */
  }
...
static const char *getobjname(const Proto *p, int lastpc, int reg, const char **name) {
  ...
  else if (lastpc != -1) { /* could find instruction? */ Instruction i = p->code[lastpc];
    OpCode op = GET_OPCODE(i);
    switch (op) {
    case OP_GETTABUP: {
...
static const char *funcnamefromcall(lua_State *L, CallInfo *ci, const char **name) {
  ...
  } else if (isLua(ci)) return funcnamefromcode(L, ci_func(ci)->p, currentpc(ci), name);
  else return NULL;
}
...
static const char *formatvarinfo(lua_State *L, const char *kind, const char *name) {
  if (kind == NULL) return ""; /* no information */
...
l_noret luaG_ordererror(lua_State *L, const TValue *p1, const TValue *p2) {
  ...
  if (strcmp(t1, t2) == 0) luaG_runerror(L, "attempt to compare two %s values", t1);
...
l_noret luaG_runerror(lua_State *L, const char *fmt, ...) {
  ...
  else if (isLua(ci)) { /* Lua function? */ /* add source:line information */
...
int luaG_tracecall(lua_State *L) {
  ...
  if (ci->u.l.savedpc == p->code) {                                                       /* first instruction (not resuming)? */
    if (p->flag & PF_ISVARARG) return 0;                                                  /* hooks will start at VARARGPREP instruction */
    else if (!(ci->callstatus & CIST_HOOKYIELD)) /* not yielded? */ luaD_hookcall(L, ci); /* check 'call' hook */
  }
...
int luaG_traceexec(lua_State *L, const Instruction *pc) {
  ...
  else if (!(mask & LUA_MASKLINE)) return 1; /* no line hook and count != 0; nothing to be done now */
